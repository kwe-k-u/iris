...
if __name__ == "__main__":
	debug = False
	if len(sys.argv) < 1:
		send_notification("Usage: python video_processor_worker.py <video_url> <show>")
		sys.exit(1)
	...
	# model = YOLO(os.path.join(os.path.dirname(__file__), '../../yolo11m.pt'))

	# if not os.path.exists(video_url) and video_url == "0":
	...
	while cap.isOpened():
		...
		# Run inference
		results = model.track(frame, conf=0.5,classes=list(detect_classes.keys()))
